...
from transformations.remove_background import remove_background
from transformations.upscale import upscale
from transformations.remove_object import remove_object
import uvicorn
...
async def transform(request: Request, transform_request: TransformRequest, x_api_key: str = Depends(verify_api_key)):
    ...
    try:
        if transform_request.transformationType == "erase_background":
            result_image = remove_background(image_data)
        elif transform_request.transformationType == "upscale":
            result_image = upscale(image_data, transform_request.upscale_factor)
            print(transform_request.upscale_factor)
        elif transform_request.transformationType == "remove_object":
            result_image = remove_object(image_data)
        else:
